...
1 cycle ahead the withdrawal's request index. This can allow malicious users to withdraw their funds early (the open period of next cycle rather than `open period of cycle after next cycle` as specified in the docs), and also if a normal user use normal time to assume his withdrawal time, he can make withdrawal his fund 1 cycle late and then he need to request withdrawal again; in edge cases, this can lead to users fund locked forever since cycle index is not tracked correctly.

https://github.com/sherlock-audit/2023-02-carapace/blob/main/contracts/core/pool/ProtectionPool.sol#L1061-#L1097

## Vulnerability Detail
The POC for this vulnerability
```typescript
    ...
    describe("...vulnerability in withdrawal demonstration", async () => {
      ...
      const _underlyingAmount: BigNumber = parseUSDC(_depositAmount);
        describe("...deposit", async () => {
          ...
          // current cycle
          const currentPoolCycleBefore =
              await protectionPoolCycleManager.getCurrentCycleIndex(
                  protectionPool.address
              );
          console.log(`Current pool cycle index ${currentPoolCycleBefore}`);
          // Move 31 days to new cycle, after this, we are now at cycle 2, but the index is still 0
          await moveForwardTimeByDays(31);
          expect(await protectionPoolCycleManager.getCurrentCycleIndex(
              protectionPool.address
          )).to.eq(0);

          // Request withdrawal at cycle 2 for deployer and seller
          await expect(
              protectionPool
          ...
          /**
           We are currently at cycle 1, currently, according to the doc:
          ...
          // Move to cycle 3 and update cycle
...
## Impact
- Malicious user can withdraw their fund 1 cycle early
- Users's fund could be locked forever
...
## Recommendation
I recommend using function `calculateAndSetPoolCycleState` every time user request withdrawal
